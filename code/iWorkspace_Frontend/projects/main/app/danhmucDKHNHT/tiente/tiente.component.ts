...
import { filterDTO, TienTe, TienTeService, FilterToolBarComponent, FilterToolBarModule, TranslatePipeModule,
...
import { CapNhatTienTeComponent } from './capnhat/capnhat.component';

@Component({
  templateUrl: './tiente.component.html',
  styleUrls: ['./tiente.component.scss'],
  providers: [TienTeService],
})
export class TienTeComponent {
  ...
  readonly linkBreadCrumb = CommonConst.TIENTE;
  ...
  dataSource: DataSource = new DataSource({
    store: new CustomStore({
      ...
      load: () => {
        ...
        return lastValueFrom(this.service.LayDSTienTe(params))
          .then((data: filterDTO<TienTe>) => {
      ...
      remove: (key: string) =>  {
        if (key !== null && key !== undefined) {
          return lastValueFrom(this.service.XoaTienTe(key))
            .then((response) => {
  ...
  constructor(private service: TienTeService, private router: Router) {
  ...
  onRowClick(e: any) {
    this.router.navigate([CommonConst.SUA_TIENTE, e.data.guid]);
  }

  onEditClick(e: any) {
    e.event.preventDefault();
    this.router.navigate([CommonConst.SUA_TIENTE, e.row.data.guid]);
  }

  addCurrency() {
    this.router.navigate([CommonConst.THEM_TIENTE]);
  };
...
@NgModule({
    ...
    providers: [],
    exports: [],
    declarations: [TienTeComponent, CapNhatTienTeComponent],
})
export class TienTeModule { }
